...
{
    private static readonly string Tag = "FogWithDepthTexture";
    private static readonly int MainTexID = Shader.PropertyToID("_MainTex");        //设置渲染的主纹理
    private static readonly int tempTexID = Shader.PropertyToID("_FogWithDepthTexture");    //定义临时的存储的RT纹理 
    
    private Material m_material;
    private FogWithDepthTextureVolume volume;
    
    private RenderTargetIdentifier source;
    
    public FogWithDepthTexturePass(RenderPassEvent renderEvent, Shader m_shader)
    {
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        volume = stack.GetComponent<FogWithDepthTextureVolume>();   //获取Volume上的组件上添加的脚本
        if (volume == null)
        {
