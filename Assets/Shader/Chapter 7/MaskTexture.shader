    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                //计算世界空间的法线
                half3 worldNormal = normalize(TransformTangentToWorld(normalTS,float3x3(i.tangentWS,i.bitangentWS,i.normalWS),true));

                half4 baseMap = SAMPLE_TEXTURE2D(_BaseMap,sampler_BaseMap,i.uv);
                half4 specularMask = SAMPLE_TEXTURE2D(_SpecularMask , sampler_SpecularMask , i.uv);

                half4 diffuse = lightColor * _DiffuseColor * baseMap * max(0.0,dot(worldNormal,worldLightDir)*0.5+0.5);
                half4 specular = lightColor * _SpecularColor * pow(max(0.0,dot(worldNormal,halfDir)),_SpecularPower) * specularMask.r * _SpecularScale;
